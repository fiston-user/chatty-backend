...
export class Add {
  ...
  public async message(req: Request, res: Response): Promise<void> {
    ...
    res.status(HTTP_STATUS.OK).json({ message: 'Message sent successfully', conversationId: conversationObjectId });
  }
  ...
  public async addChatUsers(req: Request, res: Response): Promise<void> {
    ...
    res.status(HTTP_STATUS.OK).json({ message: 'Chat users added successfully' });
  }
  ...
  public async removeChatUsers(req: Request, res: Response): Promise<void> {
    ...
    res.status(HTTP_STATUS.OK).json({ message: 'Chat users removed successfully' });
  }
  ...
  private async messageNotification({ currentUser, message, receiverName, receiverId }: IMessageNotification): Promise<void> {
    ...
    if (cachedUser.notifications.messages) {
      const templateParams: INotificationTemplate = {
        ...
        message,
        header: `New Message from ${currentUser.username}`
      };
      const template: string = notificationTemplate.notificationMessageTemplate(templateParams);
      emailQueue.addEmailJob('directMessageEmail', {
        receiverEmail: currentUser.email,
        template,
        subject: `You have a new message from ${currentUser.username}`
      });
    }
  }
}
