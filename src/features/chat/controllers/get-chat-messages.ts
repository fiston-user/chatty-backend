...
export class Get {
  ...
  public async messages(req: Request, res: Response): Promise<void> {
    ...
    const chachedMessages: IMessageData[] = await messageCache.getChatMessagesFromCache(`${req.currentUser!.userId}`, `${receiverId}`);
    if (chachedMessages.length) {
      messages = chachedMessages;
    } else {
