...
export class Get {
  ...
  public async userFollowers(req: Request, res: Response): Promise<void> {
    ...
    const cachedFollowees: IFollowerData[] = await followerCache.getFollowersFromCache(`followers:${req.params.userId}`);
    const followers: IFollowerData[] = cachedFollowees.length ? cachedFollowees : await followerService.getFollowerData(userObjectId);
