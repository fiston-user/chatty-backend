...
export class Get {
  public async comments(req: Request, res: Response): Promise<void> {
    ...
    const cachedComments: ICommentDocument[] | null = await commentCache.getCommentsFromCache(postId);
  ...
  public async singleComment(req: Request, res: Response): Promise<void> {
    ...
    const cachedComments: ICommentDocument[] | null = await commentCache.getSingleCommentFromCache(postId, commentId);
    const comments: ICommentDocument[] = cachedComments.length
      ? cachedComments
