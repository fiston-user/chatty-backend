  ...
  public instance(): Router {
    this.router.get('/instance', async (req: Request, res: Response) => {
      ...
      res
        ...
        .send(`Server is running on instance: ${response.data} with process id: ${process.pid} on ${moment().format('LL')}`);
    });
    return this.router;
  }
  ...
  public fiboRoutes(): Router {
    this.router.get('/fibo/:num', async (req: Request, res: Response) => {
      ...
      res
        .status(HTTP_STATUS.OK)
        .send(
          `Server is running on instance: ${response.data} with process id: ${process.pid} on ${moment().format('LL')}. The result of the fibonacci sequence for ${num} is ${result}. It took ${end - start} milliseconds.`
        );
    });
    return this.router;
  }
