...
describe('Settings', () => {
  ...
  describe('update', () => {
    it('should call "addUserJob" methods', async () => {
      ...
      expect(userQueue.addUserJob).toHaveBeenCalledWith('updateNotificationSettingsInDB', {
        key: `${req.currentUser?.userId}`,
        value: req.body
      });
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({ message: 'Notification settings updated', settings: req.body });
    });
  });
});
