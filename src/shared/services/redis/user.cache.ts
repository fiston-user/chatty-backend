...
export class UserCache extends BaseCache {
  ...
  public async getUsersFromCache(start: number, end: number, excludedUserKey: string): Promise<IUserDocument[]> {
    try {
      ...
      const replies: UserCacheMultiType = (await multi.exec()) as unknown as UserCacheMultiType;
  ...
  public async getRandomUsersFromCache(userId: string, excludedUsername: string): Promise<IUserDocument[]> {
    try {
      ...
      const execludedUsernamIndex: number = findIndex(replies, ['username', excludedUsername]);
      replies.splice(execludedUsernamIndex, 1);
  ...
  public async getTotalUsersInCache(): Promise<number> {
    try {
      ...
      const count: number = (await this.client.ZCARD('user')) as number;
      return count;
    } catch (error) {
      throw new InternalServerError('Server error. Try again.');
    }
