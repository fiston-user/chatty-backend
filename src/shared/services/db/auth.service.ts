...
class AuthService {
  public async createAuthUser(userData: IAuthDocument): Promise<void> {
    await AuthModel.create(userData);
  }

  public async updatePasswordToken(authId: string, token: string, expires: number): Promise<void> {
    await AuthModel.updateOne({ _id: authId }, { passwordResetToken: token, passwordResetExpires: expires });
  }
