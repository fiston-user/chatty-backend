...
class FollowerService {
  public async addFollowerToDB(userId: string, followeeId: string, username: string, followerDocumentId: ObjectId): Promise<void> {
    ...
    const users: Promise<BulkWriteResult | any> = UserModel.bulkWrite([
      {
        updateOne: {
          filter: { _id: userId },
    ...
    if (response[1]?.notifications.follows && userId !== followeeId) {
      ...
      emailQueue.addEmailJob('followersEmail', { receiverEmail: response[1].email!, template, subject: 'New follower' });
    }
  }

  public async removeFollowerFomDB(followeeId: string, followerId: string): Promise<void> {
    const followeeObjectId: ObjectId = new mongoose.Types.ObjectId(followeeId);
    ...
    const users: Promise<BulkWriteResult | unknown> = UserModel.bulkWrite([
      {
        updateOne: {
